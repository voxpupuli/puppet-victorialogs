...
#   The path where the VictoriaLogs binary will be placed when installing from archive.
# @param package_binary
#   The path where the VictoriaLogs binary will be placed when installing from package.
# @param install_dir
#   The directory where VictoriaLogs will be extracted when using archive installation.
# @param tmp_dir
...
class victorialogs::install (
  ...
  case $victorialogs::install_method {
    'archive': {
      ...
      $binary_path = $victorialogs::binary_path.lest || { $archive_binary }
      ...
      file { $install_dir:
        ...
        group   => 'root',
        mode    => '0755',
        recurse => if $victorialogs::ensure == 'absent' { true } else { undef },
        force   => if $victorialogs::ensure == 'absent' { true } else { undef },
      }

      unless $victorialogs::ensure == 'absent' {
        archive { "${tmp_dir}/victorialogs-${victorialogs::version}-${victorialogs::edition}.tar.gz":
    ...
    'package': {
      # This variable is used outside of this class to find the binary
      $binary_path = $victorialogs::binary_path.lest || { $package_binary }
